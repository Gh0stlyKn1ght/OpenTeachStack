...
- **Start with the domains.** These are your major units or themes.
- **Read the clusters.** These group related standards and suggest natural lesson groupings.
- **Read individual standards carefully.** Pay attention to the verbs — they tell you the cognitive level expected.
- **Ignore the numbering order.** Standards are numbered for reference, not for teaching sequence. You decide the order.
...
For each lesson, ask:
- Which learning target(s) does this lesson address?
- Which standard(s) do those targets unpack?
- Does the lesson activity require the same verb the standard uses?
- Will students produce something I can evaluate against this standard?
...
- OpenStax, LibreTexts, MIT OpenCourseWare, or your state-approved open resources for standards-aligned references
...
- Find your state standards document for the relevant subject and grade level
- Identify 3-5 standards that belong in one unit
- Unpack each standard into 1-3 learning targets
- For each target, write one assessment task that measures it
- Estimate the number of instructional days the unit requires
