...
The **One-Day Lesson Site Workflow** is a practical Teaching Teachers move: gather
...
- **Start with the learning need.** Name the topic, grade/course, time
   available, target skill, and one thing students should leave able to do.
- **Gather trusted sources.** Use official documentation, state standards,
...
- **Extract the teachable structure.** Pull out the key idea, vocabulary,
   examples, non-examples, diagram need, student task, check for understanding,
   and exit ticket.
- **Build a simple page.** Plain HTML and CSS is enough. Make it readable on a
   projector and student devices.
- **Embed media carefully.** Use videos and images only when they directly
   clarify the lesson. Add source notes and attribution where needed.
- **Review before teaching.** Do not invent sources, fake citations, claim
   standards alignment without evidence, or include private student information.
- **Improve after class.** Note what worked, where students got stuck, what
...
- **Factual errors** — AI can state incorrect dates, misattribute quotes, or describe processes inaccurately. Verify claims against authoritative sources.
- **Misaligned standards** — AI can reference a standard code that does not match the content it generated. Always check the actual standard text.
- **Inappropriate difficulty** — AI may write content above or below the target grade level. Read it as your students would.
- **Generic filler** — Phrases like "In this lesson, students will explore the fascinating world of..." are filler. Delete them.
- **Missing context** — AI does not know your students. It cannot account for cultural context, prior knowledge gaps, or classroom dynamics.
...
- **First prompt** — Get a draft
- **Review** — Identify what works and what does not
- **Follow-up prompt** — "Revise questions 3 and 7 to target application level instead of recall" or "Rewrite the opening hook to use an analogy about school lunch lines"
- **Review again** — Check the revisions
- **Edit manually** — Make final adjustments yourself
...
- A lesson plan prompt (include all five components: role, task, context, constraints, format)
- A rubric or assessment prompt
- A quiz or discussion question prompt
