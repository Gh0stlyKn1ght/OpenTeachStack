...
console.log(
  `${mode}Authored OTS-220 section files: ${updated}; preserved ${skipped} existing authored files.${
    force ? " (--force)" : ""
  }`,
);
if (!dryRun && skipped > 0 && !force) {
  console.log(
    `${skipped} non-authored or non-scaffolded files were preserved. Use --force to overwrite.`,
  );
}
