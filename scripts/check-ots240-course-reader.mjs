...
const courseRoot = join(root, "content", "courses", "ots-240");
...
function fail(message) {
  console.error(`OTS-240 reader migration violation: ${message}`);
  process.exitCode = 1;
}
...
if (!existsSync(courseJsonPath)) {
  fail("content/courses/ots-240/course.json is missing.");
} else {
...
const pageRoutePath = join(root, "src", "app", "book", "ots-240", "page.tsx");
if (!existsSync(pageRoutePath)) {
  fail("OTS-240 book page route is missing.");
} else {
  const routeSource = readFileSync(pageRoutePath, "utf8");
  if (!routeSource.includes("CourseStructureBookShell")) {
    fail("OTS-240 book page route is not using course structure reader wrapper.");
  }
}

const chapterRoutePath = join(root, "src", "app", "book", "ots-240", "[chapter]", "page.tsx");
if (!existsSync(chapterRoutePath)) {
  fail("OTS-240 chapter route is missing.");
} else {
  ...
  for (const fragment of ["CourseStructureBookShell", "getCourseStructure", "getChapterHref", "getSectionHref"]) {
    if (!routeSource.includes(fragment)) {
      fail(`OTS-240 chapter route missing required implementation fragment: ${fragment}`);
    }
  }
}

const sectionRoutePath = join(root, "src", "app", "book", "ots-240", "[chapter]", "[section]", "page.tsx");
if (!existsSync(sectionRoutePath)) {
  fail("OTS-240 section route is missing.");
} else {
  const routeSource = readFileSync(sectionRoutePath, "utf8");
  for (const fragment of routeForbiddenFragments) {
    if (routeSource.includes(fragment)) {
      fail(`OTS-240 section route still contains fallback/rendering fragment: ${fragment}`);
    }
  }
  for (const fragment of ["getCourseLessonBySlugs", "CourseStructureBookShell", "MDXRemote"]) {
    if (!routeSource.includes(fragment)) {
      fail(`OTS-240 section route missing required implementation fragment: ${fragment}`);
    }
  }
}
...
if (!existsSync(lessonRoot)) {
  fail("content/courses/ots-240/lessons is missing.");
} else {
  for (const filePath of walkMdx(lessonRoot)) {
    ...
    if (!parsed.data.canonicalRoute?.startsWith("/book/ots-240/")) {
...
console.log("OTS-240 course reader migration check passed.");
