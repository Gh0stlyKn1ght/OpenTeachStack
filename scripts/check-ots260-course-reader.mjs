...
const courseRoot = join(root, "content", "courses", "ots-260");
...
function fail(message) {
  console.error(`OTS-260 reader migration violation: ${message}`);
  process.exitCode = 1;
}
...
if (!existsSync(courseJsonPath)) {
  fail("content/courses/ots-260/course.json is missing.");
} else {
...
const routePath = join(root, "src", "app", "book", "ots-260", "[chapter]", "[section]", "page.tsx");
if (!existsSync(routePath)) {
  fail("OTS-260 section route is missing.");
} else {
  const routeSource = readFileSync(routePath, "utf8");
  for (const fragment of routeForbiddenFragments) {
    if (routeSource.includes(fragment)) {
      fail(`OTS-260 section route still contains registry/fallback rendering: ${fragment}`);
    }
  }
}
...
if (!existsSync(lessonRoot)) {
  fail("content/courses/ots-260/lessons is missing.");
} else {
  for (const filePath of walkMdx(lessonRoot)) {
    ...
    if (!parsed.data.canonicalRoute?.startsWith("/book/ots-260/")) {
...
console.log("OTS-260 course reader migration check passed.");
