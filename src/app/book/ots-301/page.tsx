...
export default async function Ots301BookPage() {
  const course = getCourseStructure(COURSE_SLUG);
    ...
    >
      ...
      <section className="book-spread">
        <div>
          ...
          <p>{course.thesis}</p>
          <p>
            OTS-301 focuses on building a teachable, maintainable course site with clear
            governance, safe publication habits, and accessible navigation.
          </p>
        </div>
        <ArtifactCard
          title={course.finalArtifact}
          description={`Finish when teachers can publish a public course hub with source notes, accessibility checks, and a maintenance plan.`}
        />
      </section>
