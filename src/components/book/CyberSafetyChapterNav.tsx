...
export default function CyberSafetyChapterNav({ slug }: { slug: string }) {
  const index = CYBER_BOOK_CHAPTERS.findIndex((chapter) => chapter.slug === slug);
  const previous = index > 0 ? CYBER_BOOK_CHAPTERS[index - 1] : undefined;
  const next = index >= 0 ? CYBER_BOOK_CHAPTERS[index + 1] : undefined;
